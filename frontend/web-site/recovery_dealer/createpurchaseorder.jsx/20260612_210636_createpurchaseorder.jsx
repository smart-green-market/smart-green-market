...
export default function CreatePurchaseOrder({ onClose, onSuccess }) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const formattedProducts = productList.map((p) => ({
          ...
          price: p.price || Math.floor(Math.random() * 5 + 2) * 10000,
        ...
        console.error("Lỗi khi tải dữ liệu cho đơn nhập mới:", error);
  ...
  const vatAmount = Math.round(rawSubtotal * 0.1); // VAT mặc định 10%
  const discountAmount = Math.round(rawSubtotal * 0.05); // Chiết khấu mặc định 5%
  const finalTotal = rawSubtotal + vatAmount - discountAmount;
  ...
  const handleCreateOrder = () => {
    ...
    const draftData = {
      ...
      delivery_address:
        user?.dealer_profile?.store_address || "Kho hàng đại lý đối tác",
      ...
      receiver_name:
        ...
        "Đại diện Đại lý",
      receiver_phone: user?.phone || "0901234567",
      note: "Đặt hàng nhanh từ hệ thống website",
      ...
      deposit_amount: Math.round(finalTotal * 0.3),
      debt_amount: finalTotal - Math.round(finalTotal * 0.3),
    };
