...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="p-6 text-center text-red-500 font-bold bg-white rounded-2xl border border-red-100 shadow-sm m-6">
          <h2 className="text-lg mb-2">Đã xảy ra lỗi hiển thị giao diện</h2>
          <p className="text-sm text-neutral-600 font-medium mb-4">{this.state.error?.message}</p>
          <pre className="text-xs text-left bg-neutral-50 p-4 rounded-xl overflow-auto max-w-full text-red-700 font-mono">
...
const mapSupplierInfo = (data) => {
  ...
  const formatScale = () => {
    ...
    if (!isNaN(cap) && cap > 0) {
      return `${cap.toLocaleString("vi-VN")} kg/ngày`;
    }
