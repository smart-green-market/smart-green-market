...
export default function InventoryStatsCards({ inventory }) {
  const stats = [
    ...
    },
    {
      icon: AlertTriangle,
      label: "Tồn kho thấp",
      value: inventory.filter((s) => s.status === "Sắp hết hàng").length,
      iconBg: "bg-amber-50",
    ...
    },
    {
      icon: XCircle,
      label: "Hết hàng",
      value: inventory.filter((s) => s.status === "Hết hàng").length,
      iconBg: "bg-red-50",
  ...
  return (
    <>
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-emerald-800/70 mt-1">
            Xem số lượng tồn kho nông sản, trạng thái tươi sạch và giá cả bán
            lẻ.
          </p>
        </div>
      ...
      {inventory.filter((s) => s.status === "Sắp hết hàng").length > 0 && (
        <div className="mb-6 bg-amber-50 border border-amber-200/50 p-4 rounded-2xl flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-amber-700/80 mt-0.5">
              Có {inventory.filter((s) => s.status === "Sắp hết hàng").length}{" "}
              mặt hàng đang dưới định mức an toàn. Hãy liên hệ nhà cung cấp để
              bổ sung trước khi hết hàng.
            </p>
          </div>
        </div>
      )}
