...
export default function DealerCategoryPage() {
    ...
    const handleCreateCategory = async (newCatData) => {
        try {
            ...
            // The backend might return the created object directly or inside a data field
            const newCat = response?.data || response;
            
            const mappedCat = {
                ...newCat,
                code: newCat.code || `CAT-${newCat.id || Math.floor(Math.random() * 1000)}`,
                status: newCat.status === 'active' ? "Đang kinh doanh" : "Tạm ngưng",
                count: "0 sản phẩm",
            };
