...
export default function InventoryHistoryTable({ data }) {
  const columns = [
        ...
        return (
          <span
            className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold ${
              isImport ? "bg-emerald-50 text-emerald-700 border border-emerald-100" : "bg-amber-50 text-amber-700 border border-amber-100"
            }`}
          >
            {isImport ? (
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex flex-col py-1.5">
          <span className="font-bold text-neutral-800 text-xs">{row.productName}</span>
          <span className="text-[10px] text-neutral-400 font-mono">Mã lô: {row.batchCode}</span>
        </div>
      ),
    },
    {
      ...
      cell: (row) => {
        const isImport = row.type === "Nhập kho";
        return (
          <span className={`font-bold text-xs ${isImport ? "text-emerald-700" : "text-amber-750"}`}>
            {isImport ? "+" : "-"}{row.quantity} {row.unit}
          </span>
        );
      },
    },
    ...
    },
    {
      ...
      width: "150px",
      cell: (row) => (
        <span className="text-neutral-600 font-semibold text-xs">{row.performer}</span>
      ),
    },
    {
      name: "Ghi chú",
