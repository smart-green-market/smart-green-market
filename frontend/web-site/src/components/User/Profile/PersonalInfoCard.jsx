...
/* ─── helpers ──────────────────────────────────────────── */
function tierLabel(note) {
    if (!note?.trim()) return "Khách hàng";
    return note.trim();
}
...
const ORDER_STATUS_MAP = {
    delivered:  { label: "Đã giao thành công", cls: "bg-emerald-100 text-green-900" },
...
/* ─── InfoCell ─────────────────────────────────────────── */
function InfoCell({ icon: Icon, label, value, accent = false, onClick, badge }) {
    ...
    return (
        <Tag
            ...
            className={[
                ...
                onClick
                    ? `cursor-pointer ${C.hoverBorder} ${C.hoverBg}`
                    : "",
            ].join(" ")}
        >
            <span className={`flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full ${C.iconBg}`}>
                <Icon className={`h-[17px] w-[17px] ${C.icon}`} />
            </span>

            <span className="min-w-0 flex-1">
                <span className="block text-[11px] leading-none text-neutral-400">{label}</span>
                <span className={`mt-1 block text-[14px] font-bold leading-snug ${accent ? C.accent : "text-zinc-900"}`}>
                    {value}
                </span>
                {badge && (
            ...
            {onClick && <ChevronRight className="h-4 w-4 flex-shrink-0 text-neutral-300" />}
        </Tag>
    );
}
...
export default function PersonalInfoCard({ profile, onEdit }) {
    ...
    const tier          = tierLabel(profile?.note);
    ...
    const loyalty       = (profile?.loyalty_points ?? 0).toLocaleString("vi-VN");
    ...
    const lastBadge = lastStatus
        ? (ORDER_STATUS_MAP[lastStatus] ?? { label: lastStatus, cls: "bg-gray-100 text-gray-600" })
        : null;
        ...
        <section className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-zinc-100">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 px-6 py-5">
                <div className="flex items-center gap-3">
                    <span className={`flex h-10 w-10 items-center justify-center rounded-full ${C.headerIcon}`}>
                        <User className={`h-5 w-5 ${C.icon}`} />
                    </span>
                    <div>
                        <h2 className="text-[17px] font-bold text-zinc-900">Thông tin cá nhân</h2>
                    ...
                    <div className="flex flex-col items-center gap-2 md:w-44 flex-shrink-0">
                        <div className="relative">
                            {avatar ? (
                                ...
                                <div className={`flex h-[112px] w-[112px] items-center justify-center rounded-full border-4 ${C.avatarBorder} ${C.avatarBg}`}>
                        ...
                        <p className="text-[15px] font-bold text-zinc-900 text-center">{fullName}</p>

                        <span className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-[12px] font-semibold ${tierStyle(tier)}`}>
                            <User className="h-3 w-3" />
                            {tier}
                        </span>

                        <p className="text-[12px] text-neutral-400">Thành viên từ {joinedAt}</p>
                    </div>
            ...
            <div className="grid grid-cols-4 divide-x divide-gray-100 border-t border-gray-100 bg-white">
                <QuickStat icon={Heart}   count={wishlist}    label="Sản phẩm yêu thích" onClick={() => {}} />
                <QuickStat icon={Ticket}  count={vouchers}    label="Voucher của tôi"     onClick={toVoucher} />
                <QuickStat icon={Package} count={inDelivery}  label="Đang giao"           onClick={() => navigate(`${paths.orderStatus}?status=shipping`)} />
                <QuickStat icon={Clock}   count={null}        label="Xem đơn hàng"        onClick={toOrders} />
            </div>
        </section>
    );
}
