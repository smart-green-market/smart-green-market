...
export default function CategoryProductList({ products = [] }) {
  ...
  const filteredProducts = products.filter((prod) => {
    const pName = prod.title || prod.supplier_product_name || "";
    const pCode = `PROD-${prod.id}`;
    const pStatus = prod.status === "active" ? "Đang kinh doanh" : "Tạm ngưng";
  ...
  const getStatusBadge = (status) => {
    if (status === "active" || status === "Đang kinh doanh") {
      return (
    ...
    <div className="bg-white rounded-2xl border border-neutral-100 p-6 shadow-sm font-['Geist',sans-serif]">
      ...
      <div className="overflow-x-auto border border-neutral-100 rounded-xl">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-emerald-50/50 border-b border-neutral-100">
              <th className="px-4 py-3 text-xs font-black text-emerald-950 uppercase tracking-wider">Mã sản phẩm</th>
              <th className="px-4 py-3 text-xs font-black text-emerald-950 uppercase tracking-wider">Hình ảnh</th>
              <th className="px-4 py-3 text-xs font-black text-emerald-950 uppercase tracking-wider">Tên nông sản</th>
              <th className="px-4 py-3 text-xs font-black text-emerald-950 uppercase tracking-wider text-center">Đơn vị tính</th>
              <th className="px-4 py-3 text-xs font-black text-emerald-950 uppercase tracking-wider text-right">Giá bán</th>
              <th className="px-4 py-3 text-xs font-black text-emerald-950 uppercase tracking-wider text-center">Trạng thái</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-neutral-50">
            {paginatedProducts.length > 0 ? (
              ...
              <tr>
                <td colSpan="6" className="px-4 py-8 text-center text-xs font-bold text-neutral-400">
