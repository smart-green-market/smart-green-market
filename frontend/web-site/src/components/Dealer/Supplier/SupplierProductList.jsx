...
export default function SupplierProductList({
  ...
  return (
    <div className="lg:col-span-3 bg-white rounded-2xl border border-neutral-100 p-6 shadow-sm font-['Geist',sans-serif]">
      ...
      {/* Product Cards Container with Loading Overlay */}
      <div className="relative min-h-[200px]">
        {productsLoading && (
          <div className="absolute inset-0 bg-white/60 z-10 flex items-center justify-center rounded-xl transition-all">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-emerald-600"></div>
          </div>
        )}

        {filteredProducts.length > 0 ? (
          <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 ${productsLoading ? 'opacity-40' : ''}`}>
      ...
      {/* Pagination Controls */}
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-neutral-100 pt-6 mt-6">
          <div className="text-xs text-neutral-500 font-medium">
            Hiển thị <span className="font-bold text-neutral-800">{Math.min((currentPage - 1) * pageSize + 1, totalCount)}</span> -{" "}
            <span className="font-bold text-neutral-800">{Math.min(currentPage * pageSize, totalCount)}</span> trong tổng số{" "}
            <span className="font-bold text-neutral-800">{totalCount}</span> sản phẩm
          </div>
          <Pagination
