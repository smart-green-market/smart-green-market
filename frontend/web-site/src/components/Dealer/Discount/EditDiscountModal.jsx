...
export default function EditDiscountModal({ isOpen, onClose, onSuccess, policyId }) {
  ...
  const fetchPolicy = async () => {
    try {
      ...
      setFormData({
        ...
        dealer_product: data.dealer_product || '',
        threshold_type: data.threshold_type || 'remaining_days',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Clean up payload
      if (payload.scope !== 'category') payload.category = null;
      if (payload.scope !== 'dealer_product') payload.dealer_product = null;
      ...
      toast.error(error.response?.data?.detail || 'Lỗi khi cập nhật chính sách');
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl flex flex-col max-h-[90vh]">
        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100 flex-shrink-0">
          <h2 className="text-xl font-bold text-gray-800">Cập nhật mã giảm giá theo hạn sử dụng</h2>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col min-h-0 overflow-hidden">
            ...
            <div className="flex-shrink-0 px-6 py-4 border-t border-gray-100 flex flex-col items-start gap-3 bg-white rounded-b-xl">
              <div className="flex items-center">
                ...
                />
                <label htmlFor="is_active_edit" className="ml-2 text-sm font-medium text-gray-700 cursor-pointer">
                  Đang kích hoạt
                </label>
              </div>
