...
import { orderService, parseOrderDetail } from "../../../services/api/orderService";
...
export default function DetailOrderModal({ isOpen, onClose, order: initialOrder, onUpdate }) {
  ...
  useEffect(() => {
    ...
    // Giữ items đã load nếu cùng đơn — tránh ghi đè bằng bản list không có items
    setOrder((prev) => {
      if (prev?.id === incoming?.id && prev?.items?.length) {
        return { ...incoming, items: prev.items };
      }
      return incoming;
    });
  ...
  }, [isOpen, initialOrder?.id, initialOrder?.items?.length]);
...
<body>
  <div class="header">
    <div class="brand">🌿 HTX Nông Sản<small>Phiếu xác nhận đơn hàng</small></div>
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4" aria-modal="true" role="dialog">
        ...
        <div
          className="relative w-full max-w-4xl bg-neutral-50 rounded-2xl shadow-2xl flex flex-col overflow-hidden"
