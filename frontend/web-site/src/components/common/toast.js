...
function showVariantToast(variant, message, options = {}) {
  ...
  const toastKey =
    variant === "danger" || variant === "reject" ? "error" : variant;
  const toastFn =
    variant === "reject" ? sonnerToast : sonnerToast[toastKey] || sonnerToast;

  return toastFn(message, {
    duration: 3500,
    className: `${style.toastClass} !border !w-fit !max-w-[min(420px,calc(100vw-2rem))]`,
    ...options,
  });
}
